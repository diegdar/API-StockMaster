...
class CategoryApiTest extends TestCase
{
     ...
     */
    public function test_it_verifies_category_endpoint_permissions(
        string $role,
        string $method,
        string $routeName,
        int $expectedStatus
        ...
        $url = match ($routeName) {
            'categories.index', 'categories.store' => route($routeName),
            default => route($routeName, $category->id),
        };
    ...
    public function test_admin_cannot_delete_category_with_products(): void
    {
        ...
        $category->products()->createMany(
            \App\Models\Product::factory()->count(3)->make()->toArray()
        );
